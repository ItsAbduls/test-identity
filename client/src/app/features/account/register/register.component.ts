    ...
    register(): void {
        if (this.registerForm.valid) {
            this._authService.register({
                ...
                password: this.registerForm.value.password?.toString() as string,
                redirectUrl: `http://localhost:4200/2fa`
            }).subscribe({
                next: _ => {
                    this._toastr.success('A verification link has been sent to your email account', 'Thank you for registering');
                    this._router.navigate(['2fa'], { state: { barcodeImageUrl: _.barcodeImageUrl, setupCode: _.setupCode, islogin: true, email: _.email } });
                },
                error: error => {
