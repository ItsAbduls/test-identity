...
export class ApiSampleService {
    ...
    getSampleData(): Observable<SampleDto[]> {
        return this._apiClientService.get(`https://localhost:5001/values/samples`);
    }

    addSampleData(sample: SampleDto): Observable<SampleDto> {
        return this._apiClientService.post(`https://localhost:5001/values/samples`, { data: sample });
    }
    getUsers(): Observable<boolean> {
        return this._apiClientService.get(`https://localhost:5001/api/account/users/me`);
    }
}
