...
using Insig.IdentityServer.Infrastructure.Extensions;
    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddIdentityServer(options => { options.Authentication.CookieLifetime = TimeSpan.FromMinutes(10); })
                ...
                {
                    opt.ConfigureDbContext = c => c.UseSqlServer(Configuration.GetConnectionString("Identity"),
                        sql => sql.MigrationsAssembly(migrationAssembly));
                })
                .AddOperationalStore(opt =>
                {
                    opt.ConfigureDbContext = o => o.UseSqlServer(Configuration.GetConnectionString("Identity"),
                        sql => sql.MigrationsAssembly(migrationAssembly));
                    // this enables automatic token cleanup. this is optional.
                    opt.EnableTokenCleanup = true;
                })
                .AddAspNetIdentity<AppUser>();
            // replce inMemory configuration https://code-maze.com/migrate-identityserver4-configuration-to-database/
            ...
            services.AddTransient<IGoogleAuthenticator2fa, GoogleAuthenticator2fa>();
            ...
            services.Configure<TwoFactorsSettings>(Configuration.GetSection("TwoFactorSecretCode"));

            services.AddSingleton<ICorsPolicyService>((container) => {
                var logger = container.GetRequiredService<ILogger<DefaultCorsPolicyService>>();
                return new DefaultCorsPolicyService(logger)
                {
                    AllowedOrigins = { Configuration["AppConfig:ClientUrl"],"http://localhost:4200"}
                };
            });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            {
                context.Response.Headers.Add("Content-Security-Policy", "frame-ancestors https://localhost:5002"); //modifies CSP to allow client url to frame content
                await next();
            });
            ...
            app.UseCors(b => b.WithOrigins(Configuration["AppConfig:ClientUrl"], "https://localhost:5001", "https://localhost:5000", "http://localhost:4300", "http://localhost:4200").AllowAnyHeader().AllowAnyMethod().AllowCredentials());
